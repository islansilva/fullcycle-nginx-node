...
app.get('/', (req,res) => {
    ...
    render +=  renderSelectDb().then(jhtml => html).catch(error => error);
...
const renderSelectDb = () => {
    return new Promise((resolve, reject) => {
        ...
        connection.query(sql, (error, results, fields) => {
            if (error) {
                return reject('<h4>Erro ao executar a consulta:</h4>' + error);
                
            }

            
            if (results.length === 0) {
                return resolve('<h4>Nenhum resultado encontrado.</h4>');
                
            }

            return resolve(results.map(row => `<h4>${row.name}</h4>`));
