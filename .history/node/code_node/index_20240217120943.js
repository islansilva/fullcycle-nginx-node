...
const renderSelectDb = async() => {
    ...
    connection.query(sql, (error, results, fields) => {
        if (error) {
            return '<h4>Erro ao executar a consulta:', error;
        }

        if (results.length === 0) {
            return '<h4>Nenhum resultado encontrado.';
        }

        return results.map(row => '<h4>${row.name}');

    });
    return '<h4>Erro ao executar a consulta:'
}
